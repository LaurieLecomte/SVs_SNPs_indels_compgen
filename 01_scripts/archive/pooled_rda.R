...
cand$ID <- sapply(X = cand$snp, FUN = function(x) (unlist(strsplit(x, split = '-'))[2]))
#cand$END <- cand$POS + 1

## Add variant ID (relevant for SVs only)
chr_pos_end_ID <- as.data.frame(fread(CHR_POS_END_ID, header = FALSE, 
                                      col.names = c('CHROM', 'POS', 'END', 'ID')))

cand <- merge(cand, chr_pos_end_ID, by = c('CHROM', 'POS', 'ID'), all = FALSE)
...
all_sites <- merge(all_sites, chr_pos_end_ID, by = c('CHROM', 'POS', 'END'))
