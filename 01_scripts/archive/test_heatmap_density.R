...
SVs_dens_win$dens_int <- cut_interval(SVs_dens_win$DENSITY, n = 10)

SNPs_dens_win <-  read.delim(SNP_DENS_WIN, header = FALSE,
                             col.names = c('CHROM', 'BIN_START', 'BIN_STOP', 'DENSITY'))
SNPs_dens_win$TYPE <- 'SNPs'
SNPs_dens_win$dens_int <- cut_interval(SNPs_dens_win$DENSITY, n = 10)

indels_dens_win <-  read.delim(INDEL_DENS_WIN, header = FALSE,
                               col.names = c('CHROM', 'BIN_START', 'BIN_STOP', 'DENSITY'))
indels_dens_win$TYPE <- 'indels'
indels_dens_win$dens_int <- cut_interval(indels_dens_win$DENSITY, n = 10)
...
# Create 'heat' categories for each variant type
chr1 <- subset(ALL_dens_win, CHROM_NUM == '01-23')

ggplot(chr1, aes("", round(x, 1))) + geom_tile(aes(fill = dens_int))

ggplot(chr1, aes(midBIN, "")) + 
  facet_grid(factor(TYPE, levels = c('SVs', 'SNPs', 'indels')) ~ CHROM_NUM, 
             scales = 'free', space = 'free_x')+
  geom_tile(aes(fill = dens_int))

#+ 
facet_grid(~CHROM_NUM) + xlab(NULL) + ylab("something") + 
