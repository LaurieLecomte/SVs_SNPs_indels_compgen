...
# manitou
# srun -p small -c 4 -J 03.2_indels_saf_maf_all -o log/03.2_indels_saf_maf_all_%j.log /bin/sh 01_scripts/03.2_indels_saf_maf_all.sh &

# valeria
# srun -p ibis_small -c 4 -J 03.2_indels_saf_maf_all -o log/03.2_indels_saf_maf_all_%j.log /bin/sh 01_scripts/03.2_indels_saf_maf_all.sh &
...
angsd -vcf-Pl $INDELS_VCF_ANGSD -nind $N_IND -P $CPU \
-fai "$GENOME".fai -anc $GENOME -ref $GENOME \
-domaf 1 -dosaf 1 -doMajorMinor 5 \
-out $ANGSD_STATS_DIR/"$(basename -s .recoded.vcf.gz $INDELS_VCF_ANGSD)"
...
#zless $ANGSD_STATS_DIR/"$(basename -s .recoded.vcf.gz $INDELS_VCF_ANGSD)".mafs.gz | awk '{ if ($7 >= 0.05 && $7 <= 0.95) { print } }' > $ANGSD_STATS_DIR/"$(basename -s .recoded.vcf.gz $INDELS_VCF_ANGSD)".maf0.05.mafs

zless $ANGSD_STATS_DIR/"$(basename -s .recoded.vcf.gz $INDELS_VCF_ANGSD)".mafs.gz | awk -v a=$MIN_MAF -v b=$MAX_MAF '{ if ($7 >= a && $7 <= b) { print } }' > $ANGSD_STATS_DIR/"$(basename -s .recoded.vcf.gz $INDELS_VCF_ANGSD)".maf"$MIN_MAF".mafs


# 3. Count number of filtered sites
wc -l $ANGSD_STATS_DIR/"$(basename -s .recoded.vcf.gz $INDELS_VCF_ANGSD)".maf"$MIN_MAF".mafs

# 4. Convert angsd output to bed
cat $ANGSD_STATS_DIR/"$(basename -s .recoded.vcf.gz $INDELS_VCF_ANGSD)".maf"$MIN_MAF".mafs | awk -v OFS='\t' '{print $1,$2-1,$2}' > $ANGSD_STATS_DIR/"$(basename -s .recoded.vcf.gz $INDELS_VCF_ANGSD)".maf"$MIN_MAF".bed
head $ANGSD_STATS_DIR/"$(basename -s .recoded.vcf.gz $INDELS_VCF_ANGSD)".maf"$MIN_MAF".bed

# 5. Output filtered sites from VCF
#bgzip $INDELS_VCF_ANGSD
#tabix "$INDELS_VCF_ANGSD".gz
bcftools view -R $ANGSD_STATS_DIR/"$(basename -s .recoded.vcf.gz $INDELS_VCF_ANGSD)".maf"$MIN_MAF".bed $INDELS_VCF_ANGSD | bcftools sort -Oz > $ANGSD_STATS_DIR/"$(basename -s .recoded.vcf.gz $INDELS_VCF_ANGSD)".maf"$MIN_MAF".vcf.gz
