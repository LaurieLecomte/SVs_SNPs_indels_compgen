...
# manitou
# srun -p small -c 1 -J 02.9_SNPs_rda_go -o log/02.9_SNPs_rda_go_%j.log /bin/sh 01_scripts/02.9_SNPs_rda_go.sh 10000 &

# valeria
# srun -p ibis_small -c 1 -J 02.9_SNPs_rda_go -o log/02.9_SNPs_rda_go_%j.log /bin/sh 01_scripts/02.9_SNPs_rda_go.sh 10000 &
...
less $RDA_DIR/SNPs_"$POP1"_"$POP2"_outliers_RDA_overlap"$OVERLAP_WIN"bp.table | cut -f5 | sort | uniq > $GO_DIR/SNPs_"$POP1"_"$POP2"_outliers_RDA_overlap"$OVERLAP_WIN"bp_outlierIDs.txt

# 3. Run GO enrichment
python 12_go/goatools/scripts/find_enrichment.py --pval=0.05 --indent \
  --obo $GO_DB \
  $GO_DIR/SNPs_"$POP1"_"$POP2"_outliers_RDA_overlap"$OVERLAP_WIN"bp_outlierIDs.txt \
  $GO_DIR/"$(basename -s .tsv $GENOME_ANNOT)".background.IDs.txt \
  $GO_ANNOT --min_overlap 0.1 \
  --outfile $GO_DIR/SNPs_"$POP1"_"$POP2"_RDA_outliers_overlap"$OVERLAP_WIN"bp_GO.csv
  
# 4. Filter results
MAX_FDR=0.1
MIN_LEVEL=1

Rscript 01_scripts/utils/filter_GO.R $GO_DIR/SNPs_"$POP1"_"$POP2"_RDA_outliers_overlap"$OVERLAP_WIN"bp_GO.csv $MAX_FDR $MIN_LEVEL 
