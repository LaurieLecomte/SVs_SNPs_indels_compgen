...
python 12_go/goatools/scripts/find_enrichment.py --pval=0.05 --indent \
  ...
  --outfile $GO_DIR/SNPs_"$POP1"_"$POP2"_RDAoutliers_overlap"$OVERLAP_WIN"bp_GO.csv
  
# 4. Filter results
MAX_FDR=0.1
MIN_LEVEL=1

Rscript 01_scripts/utils/filter_GO.R $GO_DIR/SNPs_"$POP1"_"$POP2"_RDAoutliers_overlap"$OVERLAP_WIN"bp_GO.csv $MAX_FDR $MIN_LEVEL 
