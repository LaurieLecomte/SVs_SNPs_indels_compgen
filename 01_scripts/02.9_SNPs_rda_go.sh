...
# manitou
# srun -p small -c 1 -J 02.9_SNPs_rda_go -o log/02.9_SNPs_rda_go_%j.log /bin/sh 01_scripts/02.9_SNPs_rda_go.sh 10000 3 &

# valeria
# srun -p ibis_small -c 1 -J 02.9_SNPs_rda_go -o log/02.9_SNPs_rda_go_%j.log /bin/sh 01_scripts/02.9_SNPs_rda_go.sh 10000 3 &
...
less $GO_DIR/SNPs_"$POP1"_"$POP2"_RDA_"$SD"sd_outliers_overlap"$OVERLAP_WIN"bp_GO.fdr"$MAX_FDR"_depth"$MIN_LEVEL".csv | cut -f1,4-6,8,13 > $GO_DIR/SNPs_"$POP1"_"$POP2"_RDA_"$SD"sd_outliers_overlap"$OVERLAP_WIN"bp_GO.fdr"$MAX_FDR"_depth"$MIN_LEVEL".simpl.txt
