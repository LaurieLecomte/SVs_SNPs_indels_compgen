...
do
   ...
   #tabix -p vcf "$ANGSD_FST_DIR/"$(basename -s .vcf.gz $FILT_ANGSD_VCF)".SNPsFst_"$pop1"_"$pop2".vcf.gz" -f 
   ...
   #tabix -p vcf "$ANGSD_FST_DIR/"$(basename -s .vcf.gz $RAW_SNPS_VCF)".SNPsFst_"$pop1"_"$pop2".vcf.gz" -f
   
   bcftools annotate -a $ANGSD_FST_DIR/$GROUP/"$pop1"_"$pop2".bypos.sfs.annot.gz -h $ANGSD_FST_DIR/$GROUP/"$pop1"_"$pop2".bypos.sfs.annot.hdr -c CHROM,POS,INFO/FST_"$pop1"_"$pop2" $SNPS_VCF_ANGSD -Oz --threads $CPU > "$ANGSD_FST_DIR/"$(basename -s .vcf.gz $SNPS_VCF_ANGSD)".SNPsFst_"$pop1"_"$pop2".vcf.gz"
   tabix -p vcf "$ANGSD_FST_DIR/"$(basename -s .vcf.gz $SNPS_VCF_ANGSD)".SNPsFst_"$pop1"_"$pop2".vcf.gz" -f
