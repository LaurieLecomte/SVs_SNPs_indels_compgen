...
# manitou
# srun -p small -c 1 -J 02.12_SNPs_compare_outliers -o log/02.12_SNPs_compare_outliers_%j.log /bin/sh 01_scripts/02.12_SNPs_compare_outliers.sh 10000 0.98 3 0.01 &

# valeria
# srun -p ibis_small -c 1 -J 02.12_SNPs_compare_outliers -o log/02.12_SNPs_compare_outliers_%j.log /bin/sh 01_scripts/02.12_SNPs_compare_outliers.sh 10000 0.98 3 0.01 &
...
Rscript 01_scripts/utils/compare_outliers_candidates_sites.R $FST_OUTLIERS $RDA_OUTLIERS $FISHER_OUTLIERS $OVERLAP_WIN $QUANTILE $MIN_FST $SD $MAX_QVAL $ANGSD_FST_DIR/SNPs_"$POP1"_"$POP2"_outliers_minFst"$MIN_FST"_qval"$MAX_QVAL"_shared.table $RDA_DIR/RDA_"$SD"sd_outliers_uniques.table
