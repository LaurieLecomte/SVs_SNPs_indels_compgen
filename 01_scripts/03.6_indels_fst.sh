...
# manitou
# srun -p small -c 4 -J 03.6_indels_fst -o log/03.6_indels_fst_%j.log /bin/sh 01_scripts/03.6_indels_fst.sh &

# valeria
# srun -p ibis_small -c 4 -J 03.6_indels_fst -o log/03.6_indels_fst_%j.log /bin/sh 01_scripts/03.6_indels_fst.sh &
...
do
   ...
   $REALSFS_PATH fst stats $ANGSD_FST_DIR/$GROUP/"$pop1"_"$pop2".fst.idx -P $CPU > $ANGSD_FST_DIR/$GROUP/"$pop1"_"$pop2".indels.fst
   ...
   tabix -p vcf "$ANGSD_FST_DIR/"$(basename -s .vcf.gz $RAW_INDELS_VCF)".indelsFst_"$pop1"_"$pop2".vcf.gz" -f 
