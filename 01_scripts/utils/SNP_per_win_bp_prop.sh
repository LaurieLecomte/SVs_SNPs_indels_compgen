...
# Compute bp covered by SNPs for a given window size, specified at positional arg 1
...
# manitou
# srun -p small -c 1 -J SNP_per_win_bp_prop -o log/SNP_per_win_bp_prop_%j.log /bin/sh 01_scripts/utils/SNP_per_win_bp_prop.sh 100000 &

# valeria
# srun -p ibis_small -c 1 -J SNP_per_win_bp_prop -o log/SNP_per_win_bp_prop_%j.log /bin/sh 01_scripts/utils/SNP_per_win_bp_prop.sh 100000 &
...
bedtools intersect -a 02_infos/chrs_win"$WIN_SIZE".bed -b $RDA_DIR/"$(basename -s .vcf.gz $RAW_FST_VCF)".CHR_POS_END_ID.table -wa -wb > $DENSITY_DIR/SNPs_intersect_win"$WIN_SIZE"bp.txt

# 4. Compute number of bp and prop
Rscript 01_scripts/utils/per_win_bp.R $DENSITY_DIR/SNPs_intersect_win"$WIN_SIZE"bp.txt 02_infos/OV_to_ssa.txt $WIN_SIZE