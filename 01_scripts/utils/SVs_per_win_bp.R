...
win_table <- merge(win_table, matched_SVs, by = c('CHROM', 'POS', 'ID'), all.x = TRUE)
...
#bp_per_win_noINS <- aggregate(data = win_table, 
#                        LEN ~ CHROM_NUM + midBIN, 
#                        FUN = sum)
#bp_per_win_noINS$prop <- bp_per_win_noINS$LEN/WIN_SIZE

# Export
#write.table(bp_per_win_noINS, file = paste0(unlist(strsplit(WIN_OVERLAP, split = '.noINS.txt')), '_bp_prop.txt'),
#            quote = FALSE, col.names = TRUE, row.names = FALSE, sep = "\t")


# Add info on INS if SV was matched with a known candidate
win_table$new_LEN <- ifelse(test = is.na(win_table$CAND_SVLEN), 
                yes = win_table$LEN,
                no = win_table$CAND_SVLEN)

bp_per_win <- aggregate(data = win_table, 
                              abs(new_LEN) ~ CHROM_NUM + midBIN, 
                              FUN = sum, na.rm = TRUE)
bp_per_win$prop <- bp_per_win$`abs(new_LEN)`/WIN_SIZE
...
# Compute mean
print(paste('mean number of bp covered by',  WIN_SIZE, 'bp window :', mean(bp_per_win$`abs(new_LEN)`)))
