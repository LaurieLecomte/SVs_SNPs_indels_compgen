...
# manitou
# srun -p small -c 1 -J SNPs_manhattan_plot -o log/SNPs_manhattan_plot_%j.log /bin/sh 01_scripts/utils/SNPs_manhattan_plot.sh 10000 0.97 3 0.01 &

# valeria
# srun -p ibis_small -c 1 -J SNPs_manhattan_plot -o log/SNPs_manhattan_plot_%j.log /bin/sh 01_scripts/utils/SNPs_manhattan_plot.sh 10000 0.97 3 0.01 &
...
ALL_SITES="$ANGSD_FST_DIR/"$(basename -s .vcf.gz $RAW_SNPS_VCF)".SNPsFst_"$POP1"_"$POP2".table"
INTERSECT_OUTLIERS="$ANGSD_FST_DIR/SNPs_"$POP1"_"$POP2"_outliers_minFst"$MIN_FST"_qval"$MAX_QVAL"_shared.table"
SHARED_3_OUTLIERS="$ANGSD_FST_DIR/SNPs_"$POP1"_"$POP2"_outliers_minFst"$MIN_FST"_qval"$MAX_QVAL"_RDA_"$SD"sd_shared.table"
